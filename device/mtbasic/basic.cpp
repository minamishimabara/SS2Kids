/*
 MINAMISHIMABARA Tiny BASIC for Arduino
...
void post(char* host, char* url, char *msg);
...
extern const char sws[];
extern const char leds[];
...
void copynum(char** d, short value) {
  unsigned char dig; //桁位置
  char lbuf[10];
  lbuf[6] = 0; //終端を置く
  dig = 6; //桁位置の初期値を末尾に設定
...
void mkmsg(char *url, char *msg) {
  short value; //値
  unsigned char i; //文字数
  char *p = url;

  if (*cip != I_STR) {
    err = ERR_SYNTAX; //最初はURL文字列
    return; //終了  
  }
  copystr(&p);
  if (chkcomma() == 2)
...
void basic() {
  ...
  //起動メッセージ
  newline(); //改行
  c_puts("MINAMISHIMABARA TINY BASIC"); //「MINAMISHIMABARA TINY BASIC」を表示
  newline(); //改行
